...
class WorkoutEngine {
  ...
  double _roundToIncrement(double weight, [String? exerciseId]) {
    ...
    if (exerciseId != null &&
        SystemConfig.instance.isBodyweightExercise(exerciseId)) {
  ...
  Exercise selectNextExercise(List<SetData> recentSets) {
    ...
    if (recentSets.isEmpty) {
      ...
      HWLog.event('engine_select_next_exercise_done',
          data: {'exercise': Exercise.bigSix[0].id});
  ...
  double calculatePrescribedWeight(
    ...
    final recentSets = history.where((s) => s.exerciseId == exerciseId).toList()
    ...
    if (recentSets.isEmpty) {
      ...
      final fallbackStart = defaultWeight ?? Exercise.startingWeightFor(exerciseId);
      HWLog.event('engine_calc_prescribed_weight_default', data: {
        'exerciseId': exerciseId,
        'prescribed': fallbackStart,
      });
      return _applyMinClamp(exerciseId, fallbackStart);
    }
  ...
  Map<String, double> estimateWeightsFromBenchPress(double benchPress5RM) {
    ...
    return {
      'bench': benchPress5RM,
      'overhead': _roundToIncrement(
          benchPress5RM * cfg.benchRatioFor('overhead'), 'overhead'),
      'row': _roundToIncrement(benchPress5RM * cfg.benchRatioFor('row'), 'row'),
      'squat': _roundToIncrement(
          benchPress5RM * cfg.benchRatioFor('squat'), 'squat'),
      'deadlift': _roundToIncrement(
          benchPress5RM * cfg.benchRatioFor('deadlift'), 'deadlift'),
      'pullup': _roundToIncrement(
          benchPress5RM * cfg.benchRatioFor('pullup'), 'pullup'),
    };
  }
  ...
  Future<DailyWorkout> generateDailyWorkout(List<SetData> history,
    ...
    // Persist current day assignment for sticky rotation
    try {
    ...
    for (final exercise in exercises) {
      final weight = calculatePrescribedWeight(
        exercise.id,
        history,
        defaultWeight: exercise.prescribedWeight,
      );
      ...
      plannedExercises.add(
        PlannedExercise(
          ...
          targetSets:
              exercise.setsTarget ?? 3, // Use database target or default to 3
  ...
  DailyWorkout applyExerciseAlternatives(
      DailyWorkout workout, ExerciseViewModel? exerciseViewModel) {
    ...
    for (final plannedExercise in workout.exercises) {
      ...
      final alternativeExercise =
          exerciseViewModel.getExerciseWithAlternative(originalExerciseId);

      if (alternativeExercise != null &&
          alternativeExercise.id != originalExerciseId) {
        ...
        if (cfg.altPreserveLoad &&
            (cfg.altOnSwap == 'map_from_current' ||
                (cfg as dynamic)._data?['alternatives']?['policy'] ==
                    'preserve_mapped')) {
          final ratio =
              cfg.alternativeRatio(originalExerciseId, alternativeExercise.id);
          if (ratio != null) {
            mappedWeight = _roundToIncrement(
                plannedExercise.prescribedWeight * ratio,
  ...
  Future<List<Exercise>> _selectTodaysExercisesFromDatabase(
      List<SetData> history) async {
    // If no repository, fall back to hardcoded exercises
    if (repository == null) {
      HWLog.event('workout_engine_no_repo');
      if (kDebugMode) {
        debugPrint(
            'WorkoutEngine: No repository available, using fallback exercises');
      }
    ...
    try {
      HWLog.event('workout_engine_fetch_day_start',
          data: {'dayId': workoutDayId});
      if (kDebugMode) {
        debugPrint(
            'WorkoutEngine: Fetching workout day $workoutDayId from database');
      }
      final workoutDay =
          await repository!.fetchCompleteWorkoutDay(workoutDayId);
      if (workoutDay != null && workoutDay.exercises.isNotEmpty) {
        ...
        if (kDebugMode) {
          debugPrint(
              'WorkoutEngine: Found ${workoutDay.exercises.length} exercises for workout day $workoutDayId');
        }
        return workoutDay.exercises.map((de) => de.exercise).toList();
      } else {
        HWLog.event('workout_engine_fetch_day_empty',
            data: {'dayId': workoutDayId});
        if (kDebugMode) {
          debugPrint(
              'WorkoutEngine: No exercises found for workout day $workoutDayId in database');
        }
      }
    } catch (e) {
      HWLog.event('workout_engine_fetch_day_error',
          data: {'dayId': workoutDayId, 'error': e.toString()});
      if (kDebugMode) {
        debugPrint(
            'WorkoutEngine: Failed to fetch workout day $workoutDayId from database: $e');
      }
    }
...
class PlannedExercise {
  ...
  factory PlannedExercise.fromJson(Map<String, dynamic> json) {
    return PlannedExercise(
      ...
      needsCalibration: json['needsCalibration'] ?? false,
    );
  }
}
