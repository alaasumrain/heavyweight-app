...
class WorkoutRepository implements WorkoutRepositoryInterface {
  ...
  Future<Map<String, SetData>> getLastForExercises(
    ...
    final result = <String, SetData>{};
    if (exerciseIds.isEmpty) return result;
    final history = await getHistory();
    for (final s in history) {
      if (!exerciseIds.contains(s.exerciseId)) continue;
      final prev = result[s.exerciseId];
      if (prev == null || s.timestamp.isAfter(prev.timestamp)) {
        result[s.exerciseId] = s;
      }
    }
    return result;
  }
