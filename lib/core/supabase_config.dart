  ...
  static Future<bool> validate() async {
    try {
      ...
      if (kDebugMode) {
        ...
        debugPrint(
            '📊 SUPABASE_URL: ${_url!.isNotEmpty ? "✅ Set (${_url!.substring(0, 20)}...)" : "❌ Empty"}');
        debugPrint(
            '📊 SUPABASE_ANON_KEY: ${_anonKey!.isNotEmpty ? "✅ Set (${_anonKey!.length} chars)" : "❌ Empty"}');
      }

      if (_url!.isEmpty) {
        if (kDebugMode) {
          debugPrint('❌ SUPABASE_URL not found in env.json');
        }
        return false;
      }
      ...
      if (_anonKey!.isEmpty) {
        if (kDebugMode) {
          debugPrint('❌ SUPABASE_ANON_KEY not found in env.json');
        }
        return false;
      }
      ...
      if (_anonKey!.length < 10) {
        if (kDebugMode) {
          debugPrint('❌ SUPABASE_ANON_KEY appears to be invalid (too short)');
        }
        return false;
      }
