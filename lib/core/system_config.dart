...
class SystemConfig {
  ...
  Map<String, dynamic> snapshot() => _data == null
      ? {}
  ...
  Future<void> load() async {
    try {
      ...
      LogConfig.update(
        ...
        cooldownsMs: (logging?['cooldownsMs'] as Map?)
            ?.map((k, v) => MapEntry(k.toString(), (v as num).toInt())),
        sampling: (logging?['sampling'] as Map?)
  ...
  void _validate() {
    ...
    if (units == null) {
      _warnings.add('MISSING_units');
    }
    // rotation
    final order = _data?['rotation']?['order'];
    if (order is! List || order.isEmpty) {
      _warnings.add('INVALID_rotation.order');
    }
    // progression
    ...
    for (final k in ['failure', 'below', 'mandate', 'exceeded']) {
      ...
      if (m == null) {
        _warnings.add('MISSING_progression.thresholds.$k.multiplier');
      }
    }
    // rest
    final base = _data?['rest']?['baseSeconds'];
    if (base == null) {
      _warnings.add('MISSING_rest.baseSeconds');
    }
    // calibration
    final maxAtt = _data?['calibration']?['maxAttempts'];
    if (maxAtt == null) {
      _warnings.add('MISSING_calibration.maxAttempts');
    }
  }
  ...
  double get roundingIncrementKg =>
      ...
      _data?['progression']?['rounding']?['incrementKg']?.toDouble() ??
      2.5;
  double get minClampDefaultKg =>
      _data?['progression']?['minClampDefaultKg']?.toDouble() ?? 20.0;
  double minClampForExerciseKg(String exerciseId) {
    final overrides =
        _data?['units']?['exerciseOverrides'] as Map<String, dynamic>?;
    if (overrides != null) {
    ...
    final fallback = Exercise.startingWeightFor(exerciseId);
    if (fallback < minClampDefaultKg) {
      return fallback;
    }
  ...
  bool get debugShortRestEnabled =>
      _debugShortOverrideEnabled ??
      (_data?['debug']?['shortRest']?['enabled'] == true);
  int get debugShortRestSeconds =>
      _debugShortOverrideSeconds ??
      (_data?['debug']?['shortRest']?['seconds']?.toInt() ?? 5);
  ...
  double get multiplierFailure =>
      _data?['progression']?['thresholds']?['failure']?['multiplier']
          ?.toDouble() ??
      0.8;
  double get multiplierBelow =>
      _data?['progression']?['thresholds']?['below']?['multiplier']
          ?.toDouble() ??
      0.95;
  double get multiplierMandate =>
      _data?['progression']?['thresholds']?['mandate']?['multiplier']
          ?.toDouble() ??
      1.0;
  double get multiplierExceeded =>
      _data?['progression']?['thresholds']?['exceeded']?['multiplier']
          ?.toDouble() ??
      1.025;

  // Calibration knobs
  int get calibMaxAttempts =>
      _data?['calibration']?['maxAttempts']?.toInt() ?? 3;
  double get calibTmRatio =>
      _data?['calibration']?['tmRatio']?.toDouble() ?? 0.90;
  double get calibWorkingFromTM =>
      _data?['calibration']?['workingFromTM']?.toDouble() ?? 0.80;
  ...
  int restForPerformance({required String category, String? exerciseId}) {
    ...
    final baseOverride = (exerciseId != null &&
            overrides != null &&
            overrides[exerciseId] != null)
  ...
  double benchRatioFor(String exerciseId) {
    final ratios =
        _data?['calibration']?['benchRatios'] as Map<String, dynamic>?;
  ...
  bool get altPreserveLoad =>
      _data?['alternatives']?['preserveLoadUsingRatio'] == true;
  String get altOnSwap =>
      (_data?['alternatives']?['onSwap'] as String?) ?? 'map_from_current';
